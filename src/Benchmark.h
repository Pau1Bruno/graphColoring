...
//–– Utility: generate 'count' symmetric sparse matrices of size n×n with given density
std::vector<SparseMatrix>
generateSparseMatrices(int n, double density, int count) {
    std::mt19937_64 rng{std::random_device{}()};
    std::uniform_real_distribution<double> dist(0.0, 1.0);
    ...
    for (int m = 0; m < count; ++m) {
        ...
        trips.reserve(static_cast<size_t>(density * n * (n-1)));
                ...
                double gaMut)
{
    ...
    auto [genSol, tX] = timeit([&]{
        GeneticColoring ga(gaPop, k, gaGen, gaMut);
        return ga.solveColoring(M.cast<double>());
    });
    int colorsX = *std::max_element(genSol.begin(), genSol.end()) + 1;
    ...
    std::cout
      ...
      << "  Genetic: colors="<<colorsX<<"  time="<<tX<<" s\n\n";
}
                   ...
                   double gaMut)
{
    for (double d : densities) {
        ...
        for (int i = 0; i < perDensity; ++i) {
            ...
            runOnDense(denseList[i], d, i,  "Dense", gaPop, gaGen, gaMut);
            // convert sparse→dense for the same algorithms
            runOnDense(sparseList[i].toDense(),
                       d, i, "Sparse", gaPop, gaGen, gaMut);
        }
    }
}
